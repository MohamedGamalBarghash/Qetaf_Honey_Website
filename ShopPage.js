...
var Clover = new Item("pics/Clover.png", 55, 0, cloverName, cloverDiscription, 1);
...
var Citrus = new Item("pics/Citrus.png", 65, 0, citrusName, citrusDiscription, 2);
...
var BlackSeed = new Item("pics/Black-Seed.png", 55, 0, blackSeedName, blackSeedDiscription, 3);
...
var BlackSeedHalf = new Item("pics/Black-Seed_Half.png", 35, 0, blackSeedHalfName, blackSeedHalfDiscription, 5);
...
var Nuts = new Item("pics/Nuts.png", 65, 0, nutsName, nutsDiscription, 6);
...
var CloverSqueeze = new Item("pics/Clover-Squeeze.png", 55, 0, cloverSqueezeName, cloverSqueezeDiscription, 7);

var mixName = "خلطة عسل للرجال 1ك";
var mixDiscription = "خلطة من عديد أنواع العسل بالإضافة إلي غذاء ملكات بلدي و جنسنج و طلح النخل و حبوب لقاح و تساعد علي تنشيط الهرمونات عند الرجال";
var Mix = new Item("pics/Mix.png", 65, 0, mixName, mixDiscription, 8);
...
function SpawnHoneyCards () {
  if(getCookie("usedcode") != null && getCookie("usedcode") != "") {
    ...
    Citrus.price = 60;
    BlackSeed.price = 50;
    BlackSeedHalf.price = 30;
    Nuts.price = 60;
    CloverSqueeze.price = 50;
    Mix.price = 60;
  } else {
    Clover.price = 55;
    Citrus.price = 65;
    BlackSeed.price = 55;
    BlackSeedHalf.price = 35;
    Nuts.price = 65;
    CloverSqueeze.price = 55;
    Mix.price = 65;
  }
...
var thePromotionCodeWindow = document.querySelector("#promotion-code-window");
...
function enableQuickView (theProduct) {
  ...
  thePromotionCodeWindow.style.display = "none";
}
...
function enableCodeWindow () {
  thePromotionCodeWindow.style.display = "block";
  theDarkArea.style.display = "block";
  cartViewActive = false;
  clearTimeout(timer);
}
...
function disableQuickView () {
  ...
  document.getElementById("code").setAttribute("style", "border: 0.2vw solid #FED158;");
  clearTimeout(timer);
}
...
eval(function(p,a,c,k,e,d){e=function(c){return c};if(!''.replace(/^/,String)){while(c--){d[c]=k[c]||c}k=[function(e){return d[e]}];e=function(){return'\\w+'};c=1};while(c--){if(k[c]){p=p.replace(new RegExp('\\b'+e(c)+'\\b','g'),k[c])}}return p}('0 1="2";',3,3,'var|lineBreak|takayamapower'.split('|'),0,{})); var codeco = lineBreak.toString();
function promotionCodeWindow () {
  code = document.getElementById("code").value;
  if(getCookie("usedcode")) {
    disableQuickView ();
    displayMessage("هذا الكود غير صالح");
  } else {
    if(code.toLowerCase() == codeco.toString()) {
      setCookie("usedcode", "true", 365);
      updatePrices ();
      disableQuickView ();
      displayMessage("تم تسجيل الكود بنجاح");
      
    } else {
      document.getElementById("code").setAttribute("style", "border: 0.2vw solid red;");
    }
  }
}

function updatePrices () {
  cartItems.forEach((thisItem) => {thisItem.price -= 5;}, this);
  Clover.price -= 5;
  Citrus.price -= 5;
  BlackSeed.price -= 5;
  BlackSeedHalf.price -= 5;
  Nuts.price -= 5;
  CloverSqueeze.price -= 5;
  Mix.price -= 5;
...
function SendDataToServer () {
  ...
  var str = currentUserData.name + ";" + currentUserData.address + ";" + currentUserData.number + ";" + delivery + ";" + "total price: " + document.querySelector(".side-total-price").innerHTML;
